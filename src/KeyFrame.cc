...
void * KeyFrame::operator new(size_t size){
	std::cout<<"Called New KeyFrame\n";
	...
	if(error != cudaSuccess){
		std::cout<<"---- GPU Memory Allocation Failed in keyFrame.cc ------ "<<std::endl;
	}
	return storage;
}
//custom deallocator.. needs to be deallocated by cudaFree
void KeyFrame::operator delete(void * add){
	//return;
	...
	if(error != cudaSuccess){
		std::cout<<"---- GPU memory De-Allocation failed in keyFrame.cc ----"<<std::endl;
	}
